...
//---
function displayIcon(icon, text, dim) {
    //---
    let scenario = window.app.game.currentScenario
    //---
    let html = ''
    html += '<div class="d-inline-block position-relative">'
        html += '<img src="' + scenario.img + icon + '.png" width="' + dim + 'px" height="' + dim + 'px" />'
        if (text) html += '<div class="position-absolute small lh-1 text-white" style="bottom:0; right:0;">' + text + '</div>'
    html += '</div>'
    return html
}